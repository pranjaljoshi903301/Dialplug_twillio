<form id="filterForm">
    ...
    <div class="widget widget-categories">
        <h3 class="widget-title">@lang('corals-classified-master::labels.partial.select_category')</h3>
    ...
    <section class="widget widget-filter-padding">
        <div class="column">
            <div class="Show-item">
                <label for="sorting">@lang('corals-classified-master::labels.partial.select_location') </label>
                <label>
                    <select name="location" id="location" class="form-control @yield('class_search')"
                            data-placeholder="Select Location">
                        <option value="">@lang('corals-classified-master::labels.partial.all_locations')</option>
    ...
    @if(\Settings::get('classified_year_model_visible'))
        @php
            $min = \Classified::getMinYearModel()??0;
            $max= \Classified::getMaxYearModel()??99999;
        @endphp
        @if($min !== $max )
            <section class="widget widget-categories">
                <h3 class="widget-title">@lang('corals-classified-master::labels.template.product.year_range')</h3>
                <div class="price-range-slider slider-style"
                     data-min="{{ $min  }}"
                     data-max="{{ $max }}"
                     data-start-min="{{ request()->input('year_model.min', $min) }}"
                     data-start-max="{{ request()->input('year_model.max', $max) }}"
                     data-step="1">
                    ...
                    <footer class="ui-range-slider-footer mt-1">
                        <div class="ui-range-values d-flex justify-content-between">
                            <div class="ui-range-value-min"><span></span>
                                <input name="year_model[min]" type="hidden">
                            </div>
                            <div class="ui-range-value-max"><span></span>
                                <input name="year_model[max]" type="hidden">
                            </div>
                        </div>
                    </footer>
                </div>
