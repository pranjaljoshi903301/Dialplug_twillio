                    ...
                    <div class="details-box">
                        <div class="ads-details-info">
                            ...
                            @if(\Settings::get('classified_year_model_visible'))
                                <p class="mb-2">{{ $product->present('year_model') }}</p>
                            @endif
                            <div class="details-meta">
                                <span data-toggle="tooltip" data-placement="top"
                                      title="{{$product->present('created_at')}}">
                                    <a href="#"><i class="lni-alarm-clock"></i>{{$product->created_at->diffForHumans()}}</a></span>
                                <span><a href="{{ $product->getLocationSlug() }}"><i
                                                class="lni-map-marker"></i>{{$product->getLocation() }}</a></span>
                                ...
                                <span><a href="#"><i class="lni-eye"></i> @lang('corals-classified-master::labels.template.product.views_count', ['count'=>$product->visitors_count])  </a></span>
                            </div>
                        ...
                        <ul class="advertisement mb-4">
                            @if(!empty($product->condition))
                                <li>
                                    <p>
                                        <strong>@lang('corals-classified-master::labels.template.product.condition')</strong>
                                        <a
                            ...
                            @if(!empty($product->brand))
                                <li>
                                    <p><strong>@lang('corals-classified-master::labels.template.product.brand')</strong>
                                        <a
                        ...
                        <div class="ads-btn mb-4">
                            @if(\Settings::get('classified_messaging_is_enable',true) && (!user() || user()->can('create', Corals\Modules\Messaging\Models\Discussion::class)))
                                <a href="{{ url('messaging/discussions/create?user='.$product->user->hashed_id) }}"
                                   class="btn btn-common btn-reply"><i
                        ...
                        <div class="short-info">
                            ...
                            <ul>
                                <li><a href="{{url('products?user='.$product->user->hashed_id)}}"><i
                                                class="lni-users"></i> {!!  trans('corals-classified-master::labels.template.product.more_products_by',['name'=>$product->getStoreName()]) !!}
                                    </a></li>
