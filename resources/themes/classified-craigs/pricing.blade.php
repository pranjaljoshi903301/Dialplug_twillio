                        ...
                        @if(user() && user()->subscribed(null, $plan->id))
                            <a href="#"
                               class="btn btn-info {{ $plan->recommended?'btn-white-border':'btn-dark-border' }}">
                                @lang('corals-classified-craigs::labels.pricing.current_package')
                            </a>
                            <br/>
                        ...
                        @else
                            <a class="btn btn-success {{ $plan->recommended?'btn-white-border':'btn-dark-border' }}"
                               href="{{ url('subscriptions/checkout/'.$plan->hashed_id) }}">
                                @lang('corals-classified-craigs::labels.pricing.subscribe_now')
                            </a>
                        @endif
                    </div>
                </div>
...
@else
    <p class="text-center text-danger">
        <strong>@lang('corals-classified-craigs::labels.pricing.product_cannot_found')</strong></p>
@endisset
