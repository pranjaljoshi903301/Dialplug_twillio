...
class CoralsInstallation extends Command
{
    ...
    public function freshInstallation()
    {
        ...
        $commands = [['command' => 'migrate:fresh', 'args' => ['--force' => true, '--seed' => true]], 'passport:install', 'key:generate'];
    ...
    protected function scanAndInstallModules()
    {
        ...
        $modulesToInstall = $modulesToInstall->filter(function ($module) {
            ...
            if ((empty($supported_platforms) && !empty($module->platform) && $platform != $this->selectedPlatform)
        ...
        if ($modulesToInstallTypePayment->isNotEmpty()) {
            ...
            if ($selectedPayment != $this->skipOption) {
                $modulesToInstallTypePayment = $modulesToInstallTypePayment->whereIn('name', Arr::wrap($selectedPayment));
    ...
    protected function installModules($modules)
    {
        ...
        $modules->sortByDesc('score')->map(function ($module) use (&$bar) {
            $module = $module['module'];

            $this->line("\n[$module->code] installing...");

            Modules::install($module->code);

            $this->line("[$module->code] completed...");
            $bar->advance();
            $this->line('');
        });
    }
    ...
    protected function pushModuleToInstall(&$modulesToInstall, $module)
    {
        ...
        if ($requireModuleToInstall) {
            ...
            $modulesToInstall->push(['name' => $module->name, 'code' => $module->code, 'module' => $module, 'score' => 1]);
        }
    }
