...
class BarsController extends APIBaseController
{
    ...
    */
    ...
    {           
        ...
        $this->twillio_logs("twilliotest",$_REQUEST);
        ...
        {            
            ...
            if($twillio_detail)
            {
                ...
                $twillio_detail->access_token = $access_token;                
                $twillio_detail->refresh_token = $refresh_token;                
                $twillio_detail->save();                
                ...
                return view('Twillio::twillio.home',['twillio_detail'=>$twillio_detail]);
            }
            else
            {                
                $is_valid = $this->check_admin_privilage($bitrix_domain,$access_token);
                ...
                $twillio_detail->subscriber_email = $subscriber_email;
                $twillio_detail->bitrix_domain = $bitrix_domain;                
                $twillio_detail->access_token = $access_token;                
                $twillio_detail->refresh_token = $refresh_token;                
                $twillio_detail->save();                
    ...
    public function bind_telephony_event($bitrix_domain,$access_token)
    {
        ...
        curl_setopt_array($curl, array(
          CURLOPT_URL => 'https://'.$bitrix_domain.'/rest/event.bind.json?auth='.$access_token.'&event=ONEXTERNALCALLSTART&handler=https://dialplug.com/api/v1/twillio/click-to-call',
        ...
        $this->twillio_logs("twillio"," Bitrix: ".$bitrix_domain."  \n Bind response:".$response);
    ...
    {    
        ...
        $all_agents=[];
        $twillioAgents = $this->twillio_users_details($result);        
        foreach($bitrix_agents as $bitrix_agent)
        {
            ...
            if(isset($twillioAgents[$bitrix_agent['EMAIL']])){
                $bitrix_agent["agent_phone"]=$twillioAgents[$bitrix_agent['EMAIL']];
        ...
        if(!$exophones)
            return view('Twillio::twillio.home',['twillio_detail'=>$result]);
    ...
    {        
        ...
        // manage agent details
            ...
            $response = $this->get_access_token($twillio_detail->refresh_token);
            if($response['is_valid']==false)
            {
                return json_encode($response);
            }
    ...
    */
    ...
    {                        
        ...
        $this->twillio_logs('twilliodebug',$api_response);
        ...
        if(isset($_REQUEST['event']))
        {
            ...
            if(!$this->twillio_subscription_validation($result->subscriber_email)){
            ...
            $twillioAgent = $this->twillio_users_details($result,$filter);
            if(!isset($twillioAgent[$agent_detail->agent_email])){
                $api_response['message'] = "Agent with user id:".$data['USER_ID']." is not found as twillio co-worker. Please register agent details first.!";
                $this->b24notify($result,$api_response['message'],$api_response);
                return response($api_response, 400);
            }            
            ...
            $twillio_obj['from'] =  $twillioAgent[$agent_detail->agent_email];
            ...
            $status_code = $this->twillio_connect_api($twillio_obj);
            $api_response['code'] = $status_code;
            if($status_code==200){                
                $api_response['response']['is_valid'] = true;                
                $api_response['message'] = "Successfully Call Triggered";                
            }                      
            $api_response['response']['caller_id'] = $twillio_obj['callerid'];
            $api_response['response']['from'] = $twillio_obj['from'];
            $api_response['response']['to'] = $twillio_obj['to'];
    ...
    {            
        ...
        // $this->twillio_logs('twillio',"Bitrix: ".$twillio_obj['bitrix_domain']."\nTwillio ClicktoCall Error Output: ".$response);
        if($status_code!=200){
            $this->twillio_logs('twillio',"Bitrix: ".$twillio_obj['bitrix_domain']."\nTwillio ClicktoCall Error Output: ".$response);
        }
        return $status_code;
    ...
    {                        
        ...
        $response = $this->get_access_token($result->refresh_token);
        if($response['is_valid']==false){
            $this->twillio_logs('twillio','Outgoing CallBack Access Token Error Output: '.json_encode($response));
            return;
        }
        $access_token = $response['access_token'];
        $refresh_token = $response['refresh_token'];
    ...
    public function incoming_click_to_call()
    {
        ...
        $response = $this->get_access_token($result->refresh_token);
        if($response['is_valid']==false){
            $this->twillio_logs('twillio','Incoming Call Access Token Error Output: '.json_encode($response));
            return;
        }
        $access_token = $response['access_token'];
        $refresh_token = $response['refresh_token'];
        ...
        if(count($call_logs)){
            ...
            foreach($call_logs as $call_log){
                ...
                if($recording_url)
                {
                    ...
                    $attachResult = $this->bitrix_execution($twillio_detail->bitrix_domain,$access_token, "POST", "telephony.externalCall.attachRecord", array(
    ...
    {                        
        ...
        if($response['original']['status']=='success')
        {
            ...
            try{
                ...
                Mail::send('Twillio::twillio.welcome_mail', $maildata, function ($emailMessage) use ($maildata, $email,$notify_dialplug) {
    ...
    {
        return $this->twillio_view_index();
    }

    public function b24notify($result,$message,$log=null)
    {
        if(!empty($log)) $this->twillio_logs('twillio',"Bitrix: ".$result->bitrix_domain."\n LOG: ".print_r($log,true));
    ...
    public function twillio_users_details($twillio_obj,$filter='')
    {
        try
        {
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => 'https://'.$twillio_obj->twillio_api_key.':'.$twillio_obj->twillio_api_token.'@ccm-api.twillio.com/v2/accounts/'.$twillio_obj->twillio_sid.'/users?fields=devices&offset=0&limit=50'.$filter,
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
                ...
                CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                CURLOPT_CUSTOMREQUEST => 'GET',
                curl_setopt($curl, CURLOPT_USERPWD, $twillio_obj->twillio_api_key.':'.$twillio_obj->twillio_api_token)            
            ));
            ...
            foreach ($users as $user) 
                $twilliousers[$user['data']['email']] = $user['data']['devices'][0]['contact_uri'];
            return $twilliousers;
        }
        catch(\Exception $e)
        {            
    ...
    public function twillio_install(Request $request)
    {
        if(isset($_REQUEST['DOMAIN']))
        {                        
            $bitrix_domain = $_REQUEST['DOMAIN'];
            ...
            $refresh_token = $response['refresh_token'];
            $this->bind_telephony_event($bitrix_domain,$access_token);
            return view('Twillio::twillio.install');
        }
        return false;
    }
