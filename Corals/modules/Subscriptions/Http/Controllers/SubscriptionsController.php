    ...
    public function doCheckout(SubscriptionCheckoutRequest $request, Plan $plan, User $user = null)
    {
        ...
        try {
            if ($plan->exists) {
                ...
                if (!isSuperUser()) {
                    $email_user = 'service-desk@dialplug.com';
                    $domain = env('APP_URL');
                    $maildata = array(
                        'domain' => $domain,
                        'name' => user()->name . " " . user()->last_name,
                        'body' => user()->name . " " . user()->last_name . ' had subscribed to a product',
                        'logged_user_id' => user()->id,
                        'logged_user_email' => user()->email,
                        'plan_name' => $plan->name,
                        'product_name' => Product::where('id',$plan->product_id)->first()->name,
                    );
                    Mail::send('email.subscriptionCreation', $maildata, function ($emailMessage) use ($maildata, $email_user) {
                        $emailMessage->subject('Subscription Creation');
                        $emailMessage->to($email_user);
                    });
                }
