    ...
    public function processImports()
    {
        ...
        try {
            ...
            if($import->store_id){
                ...
                $conf
                    ->setCountry($store->getSettingValue('amazon_api_country', 'com'))
                    ->setAccessKey($store->getSettingValue('amazon_api_access_key', ''))
                    ->setSecretKey($store->getSettingValue('amazon_api_access_secret', ''))
                    ->setAssociateTag($store->getSettingValue('amazon_api_associate_tag', ''))
                    ->setRequest($request);
    ...
    function createMissingTags($tags, $item_tags)
    {
        ...
        foreach ($item_tags as $item_tag) {
            if (!array_key_exists($item_tag['name'], $tags)) {
                ...
                $tag->slug = \Str::slug($item_tag['name']);
                $tag->save();
