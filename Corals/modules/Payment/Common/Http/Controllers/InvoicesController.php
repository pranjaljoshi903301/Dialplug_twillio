...
class InvoicesController extends BaseController
{
	...
	$company_name = $company ? $company->company_name : '';
      return view('Payment::invoices.show', compact('invoice', 'company_name'));
    }
	...
	$company = DB::select('select * from user_company where user_id = ?', [$invoice->user_id])[0];
        $company_name = $company ? $company->company_name : '';
        $pdf = \PDF::loadView('Payment::invoices.invoice', ['invoice' => $invoice, 'company_name' => $company_name,  'PDF' => true]);
    ...
    public function sendInvoice(Request $request, Invoice $invoice)
    {
	    ...
	    $company = DB::select('select * from user_company where user_id = ?', [$invoice->user_id])[0];
            $company_name = $company ? $company->company_name : '';
            event('notifications.invoice.send_invoice', [ 'company_name' => $company_name, 'invoice' => $invoice]);
