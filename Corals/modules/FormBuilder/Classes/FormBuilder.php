    ...
    public function submit(Request $request, Form $form)
    {
        try {
            ...
            if ($success_submission_action == 'show_message') {
                return response()->json(['message' => $success_submission_content, 'class' => 'alert-success', 'level' => 'success']);
    ...
    public function getFormFieldsLabel(Form $form, $showOnListState = null)
    {
        ...
        $labeledContent = $content->filter(function ($element) use ($showOnListState) {
            ...
            if (is_null($showOnListState)) {
                ...
                return ($type && in_array($type, ['text', 'autocomplete', 'checkbox-group', 'date', 'file', 'number',
                        'radio-group', 'select', 'textarea', 'starRating']) && ($elementShowInListing === $showOnListState));
            }
        });
