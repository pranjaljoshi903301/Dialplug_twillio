...
class BarsController extends APIBaseController
{
    ...
    */
    ...
    {           
        ...
        {            
            ...
            return view('Exotel::exotel.home',['bitrix_domain'=>$_REQUEST['DOMAIN']]);
        }    
    ...
    {        
        ...
        if($request->subscriber_id==-1)     
        {
            ...
            $exotel_detail->subscriber_email = $request->email;
            $exotel_detail->bitrix_domain = $request->bitrix_domain;
        ...
        $exotel_detail->webhook = $request->webhook;
        ...
        return true;
        /* $query_data = "UPDATE exotel_db.exotel_details SET webhook='$request->webhook',exotel_sid='$request->exotel_sid',exotel_api_key='$request->exotel_api_key',exotel_api_token='$request->exotel_api_token' WHERE id='$request->subscriber_id';";$result = DB::select($query_data);       */                        
        /* $query_data = 'INSERT INTO exotel_db.exotel_details (subscriber_email,webhook,exotel_sid,exotel_api_key,exotel_api_token,bitrix_domain) VALUES ("'.$request['email'].'","'.$request['webhook'].'","'.$request['exotel_sid'].'","'.$request['exotel_api_key'].'","'.$request['exotel_api_token'].'","'.$request['bitrix_domain'].'")';$result = DB::select($query_data);  */
    ...
    {    
        ...
        if(!$bitrix_agents)
            return view("Exotel::exotel.home",['bitrix_domain'=>$result->bitrix_domain,'exotel_detail'=>$result,'error'=>'Invalid Webhook']);        
        ...
        if(!$exophones)
            return view("Exotel::exotel.home",['bitrix_domain'=>$result->bitrix_domain,'exotel_detail'=>$result,'error'=>'Invalid Exotel Details']);        
    ...
    {        
        ...
        // manage exotel details
            ...
            return view("Exotel::exotel.home",['bitrix_domain'=>$exotel_detail->bitrix_domain,'exotel_detail'=>$exotel_detail]);
    ...
    */
    ...
    public function bitrix_register_api($row)
    {
        ...
        $this->exotel_logs("data_check",json_encode($registerResult));
    ...
    {                
        ...
        if($response['original']['status']=='success')
        {
            ...
            try{
                Mail::send('Exotel::exotel.welcome_mail', $maildata, function ($emailMessage) use ($maildata, $email) {
