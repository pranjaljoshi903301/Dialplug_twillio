...
<div class="row">
    <div class="col-md-5">
        <form method="post" action="{{ route('login') }}" id="login-form" class="ajax-form login-box"
            ...
            <h3 class="margin-bottom-1x">@lang('Ecommerce::labels.auth.sign_in_start_session')</h3>
            <div class="row margin-bottom-1x">
                @if(config('services.facebook.client_id'))
                    ...
                    <div class="col-xl-4 col-md-6 col-sm-4"><a class="btn btn-sm btn-block facebook-btn"
                                    ...
                                    class="socicon-facebook"></i>&nbsp;@lang('Ecommerce::labels.auth.sign_in_facebook')
                        </a></div>
                @endif
                @if(config('services.twitter.client_id'))
                    ...
                    <div class="col-xl-4 col-md-6 col-sm-4"><a class="btn btn-sm btn-block twitter-btn"
                                    ...
                                    class="socicon-twitter"></i>&nbsp;@lang('Ecommerce::labels.auth.sign_in_twitter')
                        </a></div>
                @endif
                @if(config('services.google.client_id'))
                    <div class="col-xl-4 col-md-6 col-sm-4"><a class="btn btn-sm btn-block google-btn"
                                                               href="{{ route('auth.social', 'google') }}"><i
                                    class="socicon-googleplus"></i>&nbsp;@lang('Ecommerce::labels.auth.sign_in_google')
                        </a></div>
                @endif
            ...
            <div class="form-group mb-3 has-feedback">
                <div class="checkbox">
                    ...
                    @lang('Ecommerce::labels.auth.remember_me')
                </div>
            </div>
            <div class="text-center text-sm-right">
                <button type="submit"
                        class="btn btn-primary margin-bottom-none">@lang('Ecommerce::labels.auth.login')</button>
            </div>

        </form>
        @php  $guest_checkout =   \Settings::get('ecommerce_checkout_guest_checkout',true) @endphp
            ...
            <div class="login-box">
                <div class="text-center text-sm-right">
                    <button type="submit"
                            ...
                            id="guest_checkout">@lang('Ecommerce::labels.auth.guest_checkout')</button>
                </div>
    ...
    <div class="col-md-7">
        <form method="POST" action="{{ route('register') }}" class="ajax-form login-box"
            ...
            <h3 class="margin-bottom-1x">@lang('Ecommerce::labels.auth.no_account_register')</h3>
            {{ csrf_field() }}
                    ...
                    <div class="form-group has-feedback {{ $errors->has('terms') ? ' has-error' : '' }}">
                        <div class="checkbox icheck">
                            <label>
                                ...
                                <strong>@lang('Ecommerce::labels.auth.agree')
                                    <a href="#" data-toggle="modal" id="terms-anchor"
                                       data-target="#terms">@lang('Ecommerce::labels.auth.terms')</a>
                                </strong>
                            </label>
                        </div>
                        @if ($errors->has('terms'))
                            <span class="help-block"><strong>@lang('Ecommerce::labels.auth.accept_terms')</strong></span>
                        @endif
                    </div>
                </div>
            </div>
            <div class="row justify-content-center">
                <div class="col-12 text-center text-sm-right">
                    ...
                    >@lang('Ecommerce::labels.auth.register')</button>
                </div>
            </div>
        </form>
    </div>
