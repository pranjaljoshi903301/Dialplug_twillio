...
class Shippo implements ShippingContract
{
     ...
     * @return array
     */
    public function getAvailableShipping($to_address, $shippable_items, $shipping_rule, $user = null)
    {
        if (!$user) {
            $user = user();
        }
...
//        \Logger($shipment);
        ...
        foreach ($rates as $rate) {
            $available_rates[$this->providerName() . '|' . $rate['object_id']] = [
                ...
                'amount' => $rate['amount'],
                'description' => '',
                'shipping_rule_id' => $shipping_rule->id,
    ...
    {
        $shipping_reference = $shipping_order_item->getProprty('shippo_object_id');
