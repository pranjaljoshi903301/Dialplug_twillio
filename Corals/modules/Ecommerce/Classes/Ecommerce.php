...
class Ecommerce
{
    ...
    public function deleteProduct(Product $product, $extra_params = [])
    {
        ...
        if ($response->isSuccessful()) {
            ...
            throw new \Exception(trans('Ecommerce::exception.misc.delete_product',
    ...
    public function updateOrder(Order $order, User $user, $cart_items)
    {
        ...
        if ($response->isSuccessful()) {
            ...
            // update Order failed
            $message = trans('Ecommerce::exception.misc.update_gateway_order_failed',
                ['message' => $response->getMessage()]);
    ...
    public function payOrder(Order $order, User $user, $checkoutDetails)
    {
        ...
        if ($response->isSuccessful()) {
            ...
            \Actions::do_action('post_ecommerce_pay_orders', $this->gateway, $order, $user, $checkoutDetails);
    ...
    public function refundOrder(Order $order, $amount)
    {
        ...
        if ($response->isSuccessful()) {
            ...
            \Actions::do_action('post_ecommerce_refund_order', $this->gateway, $order, $amount);
     ...
     * @throws \Exception
     */
    public function createPaymentToken(Order $order, $params)
    {
        $amount = $order->amount;
        ...
        if ($response->isSuccessful()) {
            ...
            throw new \Exception(trans('Ecommerce::exception.misc.gateway_create_payment',
    ...
    public function checkPaymentToken($params)
    {
        ...
        if ($response->isSuccessful()) {
            ...
            throw new \Exception(trans('Ecommerce::exception.misc.gateway_check_payment_token',
                ['data' => $response->getDataText()]));
        }
    }
    ...
    public function getInvoicePayableToCompanyDetails()
    {
        $companyDetails = \Settings::get('ecommerce_company*');
        ...
        $companyDetailsKeys = [
            ...
            'ecommerce_company_email',
            'ecommerce_company_phone',
            'address' => [
                'ecommerce_company_street1',
                'ecommerce_company_city',
                'ecommerce_company_state',
                'ecommerce_company_zip',
            ],
            'ecommerce_company_country',
        ];

        foreach ($companyDetailsKeys as $key) {
            if (is_array($key)) {
                foreach ($key as $subKey) {
                    if (!empty($companyDetails[$subKey])) {
                        $display_details .= $companyDetails[$subKey] . ' ';
                    }
                }
                $display_details .= '<br/>';
            } else {
                if (!empty($companyDetails[$key])) {
                    $display_details .= $companyDetails[$key] . '<br/>';
                }
            }
        }
    ...
    {
        $object = $objectData['object_details'];
        
        $order = $object['object_class']::findOrFail($object['object_id']);
        ...
        switch ($payment_status) {
            case 'paid':
                ...
                $this->checkoutOrder($order, $objectData['gateway'], $payment_reference, $payment_status, $order_status,
                    $user);
                break;
            case 'canceled':
                $order_status = 'failed';
        ...
        return redirectTo('checkout/order-success/' . $order->hashed_id);
    }

    public function checkoutOrder($order, $gateway_name, $payment_reference, $payment_status, $order_status, $user)
    {
        $checkoutService = new CheckoutService();

        $billing = $order->billing;
        $billing_address = $billing['billing_address'];
        $shipping_address = $order->shippling['shipping_address'];
