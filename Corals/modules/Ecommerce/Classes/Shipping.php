...
class Shipping
{
    ...
    public function getAvailableShippingMethods($shipping_address, $cart, $order_total)
    {
        ...
        foreach ($shipping_roles as $shipping_role) {
            try {
                ...
                $shipping_method_rates = $shipping_method->getAvailableShipping($shipping_address, $shippable_items, $shipping_role);
    ...
    {
        list($shipping_method , $shipping_type) = explode ( '|', $shipping_order_item->getProperty('shipping_method'));
        $result = [];
        if ($shipping_method) {
            $shipping_method = \App::make('Corals\\Modules\\Ecommerce\\Classes\\Shippings\\' . $shipping_method);
            $shipping_method->initialize();
            $result = $shipping_method->createShippingTransaction($shipping_order_item);
        }
        return $result;
    }
