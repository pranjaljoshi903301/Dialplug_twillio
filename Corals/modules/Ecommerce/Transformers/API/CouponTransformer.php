...
class CouponTransformer extends APIBaseTransformer
{
    ...
    public function transform(Coupon $coupon)
    {
        ...
        if ($coupon_status == "active") {
            $status = trans('Ecommerce::attributes.coupon.status_options.active');
        } else if ($coupon_status == "pending") {
            $status = trans('Ecommerce::attributes.coupon.status_options.pending');
        } else {
            $status = trans('Ecommerce::attributes.coupon.status_options.expired');
        }
